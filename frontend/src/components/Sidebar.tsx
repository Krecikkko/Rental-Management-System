...
// Przykładowe dane użytkownika - zastąp prawdziwymi
const currentUser = {
  name: "Jan Kowalski",
  email: "jan.kowalski@example.com",
  avatar: "https://placehold.co/40x40/6366f1/ffffff?text=JK"
};
      ...
      <div className="mt-auto p-2 border-t border-gray-200 dark:border-gray-700">
        <div className="relative" ref={userMenuRef}>
          {isUserMenuOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg mx-2 overflow-hidden z-10">
              <ul className="text-sm text-gray-700 dark:text-gray-200">
                ...
                <li><Link to="/dashboard/change-password" className="flex items-center w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700"><KeyIcon className="h-5 w-5 mr-3 text-gray-500 dark:text-gray-400" /><span>{t('user_menu.change_password')}</span></Link></li>
                <li><button onClick={() => {}} className="flex items-center w-full px-4 py-2 text-red-600 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"><ArrowRightOnRectangleIcon className="h-5 w-5 mr-3" /><span>{t('user_menu.logout')}</span></button></li>
              </ul>
            </div>
          )}
          <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="w-full flex items-center p-2 text-left rounded-md transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div className="ml-3 flex-1 overflow-hidden"><p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{currentUser.name}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate">{currentUser.email}</p></div>
