...
@app.post("/register")
def register(username: str, password: str, role: str = "tenant", db: Session = Depends(auth.get_db)):
    if auth.get_user_by_username(db, username):
        raise HTTPException(status_code=400, detail="Użytkownik już istnieje")
    if role not in ["owner", "tenant"]:
        raise HTTPException(status_code=400, detail="Niepoprawna rola")
    hashed_password = auth.get_password_hash(password)
    new_user = models.User(username=username, hashed_password=hashed_password, role=role)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return {"message": f"Rejestracja udana. Rola: {role}"}
