...
def assign_tenant_to_property(
    ...
    """Assigns a user with the 'tenant' role to a property. Admin only."""
    db_property = db.get(models.Property, property_id)
    if not db_property:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Property not found")
    ...
    if not user_to_assign:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Tenant to assign not found")

    if user_to_assign.role != models.Roles.TENANT:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User must have the 'tenant' role to be assigned")

    # Sprawdzenie, czy użytkownik nie jest już przypisany
    ...
    new_assignment = models.TenantAssignment.model_validate(assignment_request)
    new_assignment.property_id = property_id
...
def unassign_tenant_from_property(
    ...
    """Removes a tenant assignment. Admin only."""
